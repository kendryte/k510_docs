...
**<font face="黑体" size="6" style="float:right">K510 V4L2 Sensor Guida per lo sviluppatore</font>**

<font face="黑体"  size=3>Versione del documento: V1.0.0</font>

<font face="黑体"  size=3>Data di pubblicazione: 2022-03-11</font>
...
Questo documento è un documento esplicativo sviluppato per il sensore K510.
...
| :----- | ------ | ---------- | ------------ |
| V1.0.0 · | Zhu Dalei | 2022-03-11 | Rilasciato SDK V1.5 |
|        |        |            |              |
|        |        |            |              |
|        |        |            |              |
|        |        |            |              |
...
Prendendo imx219 come esempio, il codice è in drivers/media/i2c/soc_camera/canaanchip/imx219_0.c  

## 1.1 Aggiungere la configurazione del sensore

- Aggiungere il seguente set di configurazioni
...
- Aggiungere la configurazione alla sequenza di modalità come segue:
    ...
    Illustrare:
    .width = 1080 è il pixel effettivo del livello di uscita del sensore.
    .height = 1920 è il numero di righe altamente valide per l'uscita del sensore.
    .hts_def = 0x0d78 - IMX219_EXP_LINES_MARGIN è il livello di uscita del sensore pixel totali meno la soglia della linea di esposizione. 0x0d78 è il livello di uscita del sensore pixel totali, che verrà utilizzato nel file vidieo.conf. IMX219_EXP_LINES_MARGIN è definire la soglia della riga di esposizione, che attualmente è 4 e può essere modificata.
    .vts_def = 0x0898 è il numero totale di linee di altezza di uscita del sensore, che viene utilizzato nel file vidieo.conf.
    .reg_list = imx219_init_tab_1080_1920_30fps è la configurazione modificata sopra.

- Aggiungi il modello di flusso su, che verrà utilizzato imx219_s_stream
...
- Aggiungi la modalità di spegnimento del flusso, che verrà utilizzata imx219_s_stream
...
## 1.2 Aumentare il guadagno di esposizione e il controllo della linea di esposizione

Aggiungere o modificare
V4L2_CID_ANALOGUE_GAIN caso:
V4L2_CID_GAIN del caso:
...
## 1.3 Modificare la configurazione di inizializzazione nella funzione probe (per imx_probe)

### 1.3.1 Modificare il numero di versione
...
### 1.3.2 Aggiungere e modificare l'analisi delle informazioni canaan in DTS
...
### 1.3.3 Modificare le informazioni di ritaglio per le modalità attualmente supportate

Ciò è coerente con le informazioni di configurazione di cui sopra.
...
### 1.3.4 Modificare il nome del sensore e altre informazioni

Questo nome del sensore verrà utilizzato nel file video_cfg.conf, mediactl_lib libreria ne ha bisogno per trovare il sensore.
...
## 1.4 Aggiungere o modificare la versione del sensore della funzione di lettura (imx219_video_probe)
...
## 1.5 Modificare la funzione di controllo del sensore

Principalmente vengono modificati il valore di esposizione predefinito e il valore iniziale della linea di esposizione (IMX219_ANALOGUE_GAIN_MIN/IMX219_DIGITAL_GAIN_MAX/IMX219_ANALOGUE_GAIN_DEFAULT, IMX219_DIGITAL_EXPOSURE_MIN/IMX219_DIGITAL_EXPOSURE_MAX/IMX219_ DIGITAL_EXPOSURE_DEFAULT)
...
## 1.6 Funzioni di accensione e accensione

Principalmente per aumentare il controllo del sensore power-up e power-up, il funzionamento della porta IO.
...
## 1.7 Funzioni set_fmt

Principalmente una modifica del fmt->format.code = MEDIA_BUS_FMT_SRGGB10_1X10 frase, il sensore è supportato raw10/raw12.
...
# 2. I parametri di configurazione dell'ISP

Il file di configurazione dell'isp è mediactl_init funzione da utilizzare, in formato json, come segue: (Il significato di ciascun parametro si riferisce al documento di registro dell'ISP)
...
Questo file è mediactl_init funzione da utilizzare, è in formato json, il formato specifico e le relative spiegazioni sono le seguenti:
...
Le informazioni che devono essere modificate sono spiegate come segue:
