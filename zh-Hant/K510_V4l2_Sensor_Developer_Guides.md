...
**<font face="黑体" size="6" style="float:right">K510 V4L2 感測器開發人員指南</font>**

<font face="黑体"  size=3>文件版本：V1.0.0</font>

<font face="黑体"  size=3>發佈日期：2022-03-11</font>
...
| 版本號 | 修改者 | 修訂日期   | 修訂說明     |
| :----- | ------ | ---------- | ------------ |
| 1.0.0 版 | 朱大雷 | 2022-03-11 | SDK V1.5發佈 |
|        |        |            |              |
|        |        |            |              |
|        |        |            |              |
|        |        |            |              |
...
以imx219為例，代碼在drivers/media/i2c/soc_camera/canaanchip/imx219_0.c  
...
- 把配置增加到modes序列里，如下：
    ...
    說明：
    .width = 1080 是sensor輸出水準有效圖元。
    .height = 1920 是sensor輸出高度有效行數。
    .hts_def = 0x0d78 - IMX219_EXP_LINES_MARGIN 是sensor輸出水準總像素減去曝光行閾值。 0x0d78是sensor輸出水準總像素，這個在vidieo.conf 檔會用到。 IMX219_EXP_LINES_MARGIN是定義曝光行閾值，目前是4，可以更改。
    .vts_def = 0x0898是sensor輸出高度的總行數，這個在vidieo.conf 檔會用到。
    .reg_list = imx219_init_tab_1080_1920_30fps 是上面所改的配置。

- 增加stream on 的mode，這個在imx219_s_stream會用到
...
- 增加stream off 的mode，這個在imx219_s_stream會用到
...
## 1.3 修改probe函數中初始化配置（以imx_probe）
...
這個sensor name會在video_cfg.conf檔用到，mediactl_lib庫里需要它找到sensor。
...
## 1.4 增加或修改sensor版本的讀取函數（imx219_video_probe）
...
主要是對預設的曝光值及曝光行的初始值修改（IMX219_ANALOGUE_GAIN_MIN/IMX219_DIGITAL_GAIN_MAX/IMX219_ANALOGUE_GAIN_DEFAULT，IMX219_DIGITAL_EXPOSURE_MIN/IMX219_DIGITAL_EXPOSURE_MAX/IMX219_ DIGITAL_EXPOSURE_DEFAULT）
...
主要是對fmt->format.code = MEDIA_BUS_FMT_SRGGB10_1X10 這句話的修改，sensor是支援raw10/raw12。
...
isp的配置檔是mediactl_init函數要使用的，是json格式的，具體如下：（每個參數的意義參看ISP的寄存器文檔）
...
這個檔是mediactl_init函數要使用的，是json格式，具體格式及相關的解釋如下：
