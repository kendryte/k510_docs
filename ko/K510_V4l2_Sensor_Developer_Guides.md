...
**<font face="黑体" size="6" style="float:right">K510 V4L2 센서 개발자 안내서</font>**

<font face="黑体"  size=3>문서 버전: V1.0.0</font>

<font face="黑体"  size=3>게시 날짜: 2022-03-11</font>
...
이 문서는 K510 sensor용으로 개발된 설명서입니다.
...
| :----- | ------ | ---------- | ------------ |
| V1.0.0 | 주다레이 | 2022-03-11 | SDK V1.5 릴리스 |
|        |        |            |              |
|        |        |            |              |
|        |        |            |              |
|        |        |            |              |
...
imx219의 경우 코드는 drivers/media/i2c/soc_camera/canaanchip/imx219_0.c입니다  
...
- 구성을 다음과 같이 모데스 시퀀스에 추가합니다.
    ...
    설명:
    .width = 1080은 sensor 출력의 수평 유효 픽셀입니다.
    .height = 1920은 sensor 출력에서 매우 유효한 행 수입니다.
    .hts_def = 0x0d78 - IMX219_EXP_LINES_MARGIN sensor 출력의 총 수평 픽셀에서 노출 행 임계값을 빼는 것입니다. 0x0d78 vidieo.conf 파일에서 사용되는 sensor 출력의 총 가로 픽셀입니다. IMX219_EXP_LINES_MARGIN 노출 행 임계값을 정의하는 것으로, 현재 4이며 변경할 수 있습니다.
    .vts_def = 0x0898 vidieo.conf 파일에 사용되는 sensor 출력 높이의 총 행 수입니다.
    .reg_list = imx219_init_tab_1080_1920_30fps 위에서 변경한 구성입니다.

- stream on의 모드가 추가되어 imx219_s_stream 사용됩니다
...
- stream off의 모드가 추가되어 imx219_s_stream 사용됩니다
...
## 1.3 probe 함수에서 초기화 구성 수정(imx_probe)
...
### 1.3.2 DTS에서 Jianan 정보 분석을 추가하고 수정합니다
...
### 1.3.3 현재 지원 모드에 대한 CROP 정보를 수정합니다
...
### 1.3.4 sensor 이름 등의 정보를 수정합니다

이 sensor name은 video_cfg.conf 파일에서 사용되며 mediactl_lib 라이브러리에서 sensor를 찾는 데 필요합니다.
...
## 1.4 sensor 버전의 읽기 함수 추가 또는 수정(imx219_video_probe)
...
## 1.5 sensor의 제어 함수를 수정합니다

주로 기본 노출 값 및 노출 행의 초기 값 수정(IMX219_ANALOGUE_GAIN_MIN/IMX219_DIGITAL_GAIN_MAX/IMX219_ANALOGUE_GAIN_DEFAULT, IMX219_DIGITAL_EXPOSURE_MIN/IMX219_DIGITAL_EXPOSURE_MAX/IMX219_ DIGITAL_EXPOSURE_DEFAULT）
...
주로 센서 상하전 제어, IO 포트의 작동을 증가시킵니다.
...
주로 fmt->format.code = MEDIA_BUS_FMT_SRGGB10_1X10 이 문장의 수정이며 sensor는 raw10/raw12를 지원합니다.
...
isp의 구성 파일은 mediactl_init 함수에서 사용할 수 있으며 다음과 같이 json 형식입니다(각 매개 변수의 의미는 ISP의 레지스터 설명서 참조).
...
이 파일은 mediactl_init 함수에서 사용할 json 형식이며 특정 형식 및 관련 해석은 다음과 같습니다.
