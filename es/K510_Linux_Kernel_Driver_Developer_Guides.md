...
Este documento es un documento de apoyo para el SDK K510, este documento habla principalmente sobre los controladores relacionados con Linux, la configuración, la depuración, etc.
...
Para obtener más información sobre el kernel de Linux, visite:<https://docs.kernel.org/>
...
## 1.2 Requisitos del entorno de desarrollo
...
# 2 Archivo de configuración predeterminado del kernel y dts

Ruta predeterminada del archivo de configuración del kernel:

arch/riscv/configs/k510_defconfig

El kernel soporta dos placas de desarrollo, K510 CRB y EVB, y los archivos dts correspondientes son los siguientes:

arch/riscv/boot/dts/canaan/k510_crb_lp3_v0_1.dts

arch/riscv/boot/dts/canaan/k510_evb_lp3_v1_1.dts

En el directorio arch/riscv/boot/dts/canaan/k510_common se encuentra la definición de dts público de nivel soc.

# 3 Depuración

## 3.1 Depurar el kernel de Linux con JTAG

1. Instalar Andesight v3.2.1
2. Vaya al directorio ice en el directorio de instalación de andesight y ejecute ICEMAN

    ```shell
    #ICEman -Z v5 --smp
    ```

3. Usando la depuración de gdb, aquí está el controlador de código del kernel /dev/mem/char/mem.c como ejemplo
...
4. La aplicación abre /dev/mem, llama a mmap y entra en el punto de interrupción

# 4 Descripción del conductor
...
API: Nodo de archivo de dispositivo:
...
Interfaz de programación: controlador de puerto serie estándar, consulte la página del comando man de Linux
...
Dispositivo:`eth0`
Descripción de la API: Controlador de puerto Ethernet estándar, consulte la programación del socket tcp/ip; 

Configuración IP del puerto Ethernet:
...
API del controlador: controlador estándar, como un archivo ordinario para leer y escribir.
...
Interfaz de programación: archivo linux IO (abrir, cerrar, ioctl), consulte la página del comando man Linux
El código fuente del kernel viene con documentación:`Documentation/watchdog/watchdog-api.txt`
...
API: se puede acceder al controlador pwm en estado de usuario a través de sysfs, `/sys/class/pwm/`

Interfaz de programación: E/S de archivos Linux (abrir, leer, escribir), consulte la página del comando man Linux

El código fuente del kernel viene con documentación:`Documentation/pwm.txt`
...
API: El controlador I2C es un controlador de bus y se implementa utilizando el marco del subsistema I2C del kernel de Linux. Se puede acceder al estado de usuario a través de sysfs, o se pueden usar programas de herramientas de estado de usuario como i2c-tools.

```shell
/sys/bus/i2c/devices/
```

Interfaz de programación: IO de archivo Linux (abrir, leer, escribir), consulte la página del comando man de Linux
El código fuente del kernel viene con documentación:`Documentation/i2c/dev-interface`
...
USB como host, se puede conectar a un disco U, como dispositivo, se puede utilizar como disco U.
...
- `clock_provider.dtsi`Definir todos los nodos de reloj en
- `clock_consumer.dtsi`Referencias en cada nodo dts del controlador
...
- `power_provider.dtsi` Se define el nodo dts del proveedor
- `include/dt-bindings/soc/canaan,k510_pm_domains.h` Todos los dominios de potencia se definen en
- `power_consumer.dtsi`Se hace referencia al <a0> en los nodos dts respectivos de los controladores
...
- `reset_provider.dtsi` Se define el nodo dts del proveedor
- `include/ dt-bindings/reset/canaan-k510-reset.h` Todas las señales de reinicio se definen en
- `reset_consumer.dtsi`Se hace referencia al <a0> en los nodos dts respectivos de los controladores
...
Árbol de dispositivos relacionados:
...
`iomux_provider.dtsi` Se define el nodo dts del proveedor
`include/include/dt-bindings/pinctrl/k510.h`Todos los números de función de E/S se definen en
`iomux_consumer.dtsi`Se hace referencia al <a0> en los respectivos nodos dts de los controladores
...
API: Nodo de archivo de dispositivo: `/dev/h264-codec`

Interfaz de programación: Linux file IO(open, close, ioctl), consulte la página del comando man de Linux

Comandos IOCTL compatibles:
...
Código de ejemplo:`package/h264_demo/src`
...
API: Nodo de archivo de dispositivo: `/dev/k510-dsp`
...
API: Nodo de archivo de dispositivo: /dev/k510-gnne
Interfaz de programación: Linux file IO(open, close, ioctl), consulte la página del comando man de Linux
Comandos ioctl compatibles:
...
API: Nodo de archivo de dispositivo: /dev/kendryte_2d
Interfaz de programación: Linux file IO(open, close, ioctl), consulte la página del comando man de Linux
Comandos ioctl compatibles:
...
API: Archivos de nodo de dispositivo:`/sys/bus/platform/devices/91000000.aes`
`/sys/bus/platform/devices/91010000.sha`

Interfaz de programación: los programas de estado de usuario utilizan sockets para acceder a la API del controlador del kernel, donde se encuentra la documentación de referencia`/Documentation/crypto/userspace-if.rst`
...
## 4.18 Monitoreo de temperatura - térmico
