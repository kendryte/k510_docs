...
**<font face="黑体" size="6" style="float:right">Guía del desarrollador de K510 V4L2</font>**
...
Este documento es un documento explicativo para el ejemplo de aplicación K510 V4L2. 
...
| v1.0.1 | Zhu Dalei | 2022-03-11 | Lanzamiento del SDK V1.5 |
|        |        |            |                    |
|        |        |            |                    |
|        |        |            |                    |
|        |        |            |                    |
...
Apague el dispositivo multimedia y libere la memoria compartida solicitada.
...
Configurar el valor AE del sensor
...
Obtiene el estado de habilitación de cada módulo del ISP.
...
# 2 Aplicación de demostración

## 2.1 v4l2_drm

El programa se coloca`/app/mediactl_lib` en el directorio:

- `v4l2_drm.out`:v4l2 y caso de vinculación drm, agregado -f para modificar el nombre del archivo de configuración de entrada, -e para abrir la función isp ae. Puede usar -h para ver la ayuda.

Ejecutar v4l2_drm.out

- -e:0 apaga todos los aes, 1 enciende f-2k ae, 2 enciende r-2k ae y 3 enciende todos los aes. De forma predeterminada, puede dejar -e para desactivar todos los aes.
- La demostración requiere que el archivo de configuración de vídeo y el archivo de configuración del sensor correspondiente estén en el directorio actual.
- La demostración puede demostrar cámaras simples y dobles cambiando el archivo de configuración.
- La demostración de demostración de una sola cámara a pantalla completa: ./v4l2_drm.out -e 1 -f video_drm_1080x1920.conf
- La demo demo de cámara dual: ./v4l2_drm.out -f video_drm_1920x1080.conf
- La demostración debe garantizar que existen tres perfiles video_drm_1920x1080.conf, imx219_0.conf y imx219_1.conf
