...
**<font face="黑体" size="6" style="float:right">K510 V4L2開発者ガイド</font>**
...
このドキュメントは、K510 V4L2 アプリケーション インスタンスの説明ドキュメントです。 
...
| v1.0.1の | ジュダレイ | 2022-03-11 | SDK V1.5 リリース |
|        |        |            |                    |
|        |        |            |                    |
|        |        |            |                    |
|        |        |            |                    |
...
media デバイスをシャットダウンし、アプリケーションの share memory メモリを解放します。
...
プログラムは、`/app/mediactl_lib`ディレクトリの下に配置されます:

- `v4l2_drm.out`:v4l2とdrm連動caseは、-f変更入力プロファイルの名前を追加し、-eはisp ae機能を開きます。 ヘルプは -h を使用して表示できます。

v4l2_drm.out を実行します

- -e:0 すべてのaeをオフにし、1はf-2k aeを開き、2はr-2k aeを開き、3はすべてのaeを開きます。 デフォルトでは、-e を指定せずにすべての ae をオフにすることができます。
- この demo には、video プロファイルと対応するsensorプロファイルが現在のディレクトリに必要です。
- demo は、プロファイルを変更することで、シングルとデュアルの両方のカメラを表示できます。
- demo は、全画面表示:./v4l2_drm.out -e 1 -f video_drm_1080x1920.conf を示しています
- demo はデュアル撮影:./v4l2_drm.out -f video_drm_1920x1080.conf を示しています
- demo は、video_drm_1920x1080.conf、imx219_0.conf、およびimx219_1.conf の 3 つのプロファイルが存在することを確認する必要があります
