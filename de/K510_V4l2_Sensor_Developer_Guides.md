...
**<font face="黑体" size="6" style="float:right">K510 V4L2 Sensor Entwicklerhandbuch</font>**

<font face="黑体"  size=3>Dokumentversion: V1.0.0</font>

<font face="黑体"  size=3>Veröffentlicht: 2022-03-11</font>
...
Dieses Dokument ist ein erläuterndes Dokument, das für den Sensor K510 entwickelt wurde.
...
| :----- | ------ | ---------- | ------------ |
| V1.0.0 | Zhu Dalei | 2022-03-11 | SDK V1.5 veröffentlicht |
|        |        |            |              |
|        |        |            |              |
|        |        |            |              |
|        |        |            |              |
...
# 1. V4L2 sesor Treiber

Am Beispiel von imx219 befindet sich der Code in drivers/media/i2c/soc_camera/canaanchip/imx219_0.c  

## 1.1 Sensorkonfiguration hinzufügen
...
- Fügen Sie die Konfiguration wie folgt zur Modussequenz hinzu:
    ...
    Illustrieren:
    .width = 1080 ist das effektive Pixel des Sensorausgangspegels.
    .height = 1920 ist die Anzahl der hochgültigen Zeilen für den Sensorausgang.
    .hts_def = 0x0d78 - IMX219_EXP_LINES_MARGIN ist der Gesamtbetrag des Sensorausgangspegels abzüglich des Schwellenwerts für die Belichtungslinie. 0x0d78 ist der Gesamtwert des Sensorausgangspegels, der in der Datei vidieo.conf verwendet wird. IMX219_EXP_LINES_MARGIN besteht darin, den Schwellenwert für die Belichtungsreihe zu definieren, der derzeit bei 4 liegt und geändert werden kann.
    .vts_def = 0x0898 ist die Gesamtzahl der Zeilen der Sensorausgabehöhe, die in der Datei vidieo.conf verwendet wird.
    .reg_list = imx219_init_tab_1080_1920_30fps ist die oben geänderte Konfiguration.

- Fügen Sie das Modell des Streams hinzu, das imx219_s_stream verwendet wird
...
- Fügen Sie den Stream-Off-Modus hinzu, der imx219_s_stream verwendet wird
...
## 1.2 Erhöhung des Belichtungsgewinns und der Kontrolle der Belichtungslinie

Hinzufügen oder Ändern
Fall V4L2_CID_ANALOGUE_GAIN:
Fall V4L2_CID_GAIN:
Fall V4L2_CID_EXPOSURE:
...
## 1.3 Ändern der Initialisierungskonfiguration in der Prüfpunktfunktion (auf imx_probe)

### 1.3.1 Ändern der Versionsnummer
...
### 1.3.2 Hinzufügen und Ändern der Analyse von Canaan-Informationen in DTS
...
### 1.3.3 Ändern der Zuschneideinformationen für derzeit unterstützte Modi

Dies steht im Einklang mit den obigen Konfigurationsinformationen.
...
### 1.3.4 Ändern des Sensornamens und anderer Informationen

Dieser Sensorname wird in video_cfg.conf-Datei verwendet, mediactl_lib Bibliothek benötigt ihn, um den Sensor zu finden.
...
## 1.4 Hinzufügen oder Ändern der Sensorversion der Lesefunktion (imx219_video_probe)
...
## 1.5 Ändern der Steuerungsfunktion des Sensors

Hauptsächlich werden der Standardbelichtungswert und der Anfangswert der Belichtungslinie geändert (IMX219_ANALOGUE_GAIN_MIN/IMX219_DIGITAL_GAIN_MAX/IMX219_ANALOGUE_GAIN_DEFAULT, IMX219_DIGITAL_EXPOSURE_MIN/IMX219_DIGITAL_EXPOSURE_MAX/IMX219_ DIGITAL_EXPOSURE_DEFAULT)
...
## 1.6 Ein- und Einschaltfunktionen

Hauptsächlich, um die Kontrolle des Sensor-Power-Up und Power-Up, des Betriebs des IO-Ports, zu erhöhen.
...
## 1.7 set_fmt Funktionen

Hauptsächlich eine Modifikation des fmt->format.code = MEDIA_BUS_FMT_SRGGB10_1X10 Satz, der Sensor wird raw10/raw12 unterstützt.
...
# 2. Die Konfigurationsparameter des ISP

Die Konfigurationsdatei von isp ist mediactl_init Funktion, die im JSON-Format wie folgt verwendet werden kann: (Die Bedeutung der einzelnen Parameter bezieht sich auf das Registerdokument des ISP)
...
Diese Datei ist mediactl_init zu verwendende Funktion, ist im JSON-Format, das spezifische Format und die zugehörigen Erklärungen sind wie folgt:
...
Die Informationen, die geändert werden müssen, werden wie folgt erläutert:
